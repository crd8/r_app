...
if((tableWidthAttr||scrollX)&&!settings._reszEvt){var resize=DataTable.util.throttle(function(){var newWidth=_fnWrapperWidth(settings);if(!settings.bDestroying&&newWidth!==0){_fnAdjustColumnSizing(settings)}});if(window.ResizeObserver){var first=$(settings.nTableWrapper).is(':visible');settings.resizeObserver=new ResizeObserver(function(e){if(first){first=!1}else{resize()}});settings.resizeObserver.observe(settings.nTableWrapper)}else{$(window).on('resize.DT-'+settings.sInstance,resize)}
settings._reszEvt=!0}}
...
cell.attr('aria-label',orderable?col.ariaTitle+ctx.api.i18n('oAria.orderable'+ariaType):col.ariaTitle);if(orderable){var orderSpan=cell.find('.dt-column-order');orderSpan.attr('role','button');if(tabIndex!==-1){orderSpan.attr('tabindex',tabIndex)}}})}},layout:{_:function(settings,container,items){var classes=settings.oClasses.layout;var row=$('<div/>').attr('id',items.id||null).addClass(items.className||classes.row).appendTo(container);DataTable.ext.renderer.layout._forLayoutRow(items,function(key,val){if(key==='id'||key==='className'){return}
